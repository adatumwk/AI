...
import re  # Добавлено для парсинга даты
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=30), retry=retry_if_exception_type((aiohttp.ClientError, ValueError)))
async def parse_horoscope(sign_id: int, base_url: str, session: aiohttp.ClientSession):
    ...
    try:
        ...
        raise ValueError(f"Ошибка парсинга HTML: {e}")
    ...
    # Извлечение даты с страницы
    date_text = soup.find(string=re.compile(r"СЕГОДНЯ \d+ \w+ \d{4}"))
    if date_text:
        # Пример: "СЕГОДНЯ 24 октября 2025" -> parse to date
        match = re.search(r"(\d+) (\w+) (\d{4})", date_text)
        if match:
            ...
            if month:
                ...
                result["parsed_date"] = date(int(year), month, int(day))  # Для future use, если нужно скорректировать
    ...
    if container:
        # Улучшение: Ищем по нескольким классам для всех sub блоков
        blocks = container.find_all("div", class_=[BUSINESS_BLOCK_CLASS, SUB_CONTAINER_CLASS])
        for block in blocks:
